...
try:
	...
	try:
		...
		from PIL import ImageEnhance #@UnusedImport
	except ImportError:
...
class GalleryUpload(models.Model):
	...
	def save(self, *args, **kwargs):
		...
		if os.path.isfile(self.zip_file.path):
			os.remove(self.zip_file.path)		# Zip delete
	...
	def process_zipfile(self):
		if os.path.isfile(self.zip_file.path):
			...
			if self.gallery:
				...
				while True:
					try:
						gallery = Gallery.objects.create(owner=self.owner,
													 title=self.title,
													 title_slug=self.u_slugify(' '.join([self.title, nowTimeSlice])),
			...
			for filename in zip.namelist():
				...
				if len(data):
					try:
						...
						continue
					# test first then duplicated; put timestamp
					try:
						photoExtSplit = path.splitext(unicode(filename,'cp949'))
					except:
						photoExtSplit = path.splitext(unicode(filename,'utf-8'))
					splitedFilename = photoExtSplit[0][0:16]
					splitedFileExt = photoExtSplit[1]
					photoTitle = ''.join([splitedFilename, splitedFileExt])
					'''try:
						photoTitle = ''.join([splitedFilename, splitedFileExt])
						photo = Photo(title=photoTitle,
								  title_slug=self.u_slugify(photoTitle),
								  caption=self.caption,
								  tags=self.tags)
						photo.image.save(photoTitle, ContentFile(data))
					except IntegrityError:
						try:
							photoTitle =  ''.join(
								[
									'_'.join([splitedFilename, nowTimeSlice])
									,splitedFileExt
								]
								)			# put timestamp
							photo = Photo(title=photoTitle,
					...
					while True:
						try:
							...
							photo = Photo(title=photoTitle,
									  title_slug=self.u_slugify(photoSlugTitle),
									  caption=self.caption,
									  tags=self.tags)
							photo.image.save(photoTitle, ContentFile(data))
							break
	...
	def u_slugify(self, txt):
		...
		txt = re.sub('\.', '_', txt, re.UNICODE) # replace dot with underscores
...
class ImageModel(models.Model):
	image = models.ImageField(_('image'), upload_to='photologue/%Y/%m/%d')
	...
	def delete(self):
		...
		if os.path.isfile(self.image.path):
			os.remove(self.image.path)		# image delete
