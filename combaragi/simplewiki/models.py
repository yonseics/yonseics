...
class Article(models.Model):
    ...
    def get_root(cls):
        ...
        try:
            if not Article.objects.count():
                Article.objects.create(title='root', slug='root', parent=None)
    ...
    def get_url_reverse(cls, path, article, return_list=[]):
        ...
        #print 'path[0]: ', path[0]
        #print Article.objects.get(parent__exact = article, slug__exact=str(path[0]))
        #print Article.objects.get(slug__exact=str(path[0]))
        try:
            a = Article.objects.get(parent = article, slug=unicode(path[0]).encode('utf-8'))
...
class Revision(models.Model):
    ...
    def save(self, **kwargs):
        ...
        ext += ["wikilinks(base_url=%s/)" % reverse('wiki_view', args=('home',))]
...
class RevisionForm(forms.ModelForm):
    contents = forms.CharField(label=u'내용', widget=forms.Textarea(attrs={'rows':8, 'cols':50}))
    revision_text = forms.CharField(label=u'바꾼내용 간추리기')
    class Meta:
        model = Revision
...
class CreateArticleForm(RevisionForm):
    title = forms.CharField(label=_('Title'))
    revision_text = forms.CharField(widget=forms.HiddenInput, required=False)
    class Meta:
        model = Revision
