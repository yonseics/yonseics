...
def root_redirect(request):
  ...
  #else:
  #  return view(request, root.slug)
  return HttpResponseRedirect(reverse('wiki_view', args=['home']))

def create(request, wiki_url):
  if wiki_url in ('/', 'root', '', None):
    return create(request, 'home')
  ...
  c = RequestContext(request, {'form': f,
                               ...
                               'wiki_article': Article.get_root(),
                 'wiki_write': True,
                 'wiki_title': request.GET.get('wiki_article_name', url_path[-1]),
                 })
...
def history(request, wiki_url, page=1):
  if wiki_url in ('/', 'root', '', None):
    return history(request, 'home', page)
...
def search_articles(request, wiki_url):
  if wiki_url in ('/', 'root', '', None):
    return search_articles(request, 'home')
  #return HttpResponse(wiki_url)
...
def search_add_related(request, wiki_url):
  if wiki_url in ('/', 'root', '', None):
    return search_add_related(request, 'home')
...
def add_related(request, wiki_url):
  if wiki_url in ('/', 'root', '', None):
    return add_related(request, 'home')
...
def remove_related(request, wiki_url, related_id):
  if wiki_url in ('/', 'root', '', None):
    return remove_related(request, 'home')
...
def random_article(request, wiki_url):
  if wiki_url in ('/', 'root', '', None):
    return random_article(request, 'home')
  from random import randint
  num_arts = Article.objects.exclude(parent=None).count()
  article = Article.objects.exclude(parent=None)[randint(0, num_arts-1)]
  return HttpResponseRedirect(reverse('wiki_view', args=(article.get_url(),)))
...
def double_slash(request, wiki_url):
  """Generate a double slash message for some URL"""
  return render_to_response('simplewiki_error.html',
                RequestContext(request, {'wiki_err_double_slash': True,
                             'wiki_url': wiki_url}))
