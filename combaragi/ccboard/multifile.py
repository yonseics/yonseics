...
class MultiFileInput(FileInput):
  ...
  def fields(self, name, value, count, attrs=None):
    ...
    """
    return u''.join([u'<p class="left"><input%s /></p>\n' % flatatt(dict(attrs, id=attrs['id']+str(i))) for i in range(count)])

  def link(self, name, value, count, attrs=None):
    """
    Renders a link to add more file input boxes.
    """
    return u"<span class='btn_pack medium'><button type='button' onclick=\"javascript:new_%(name)s()\">%(add_label)s</button></span><br>" % {
      'name':name,
      'add_label':render_button(u'추가하기')
    }
...
class MultiFileField(Field):
  ...
  def clean(self, data):
    ...
    for a_file in f:
      ...
      if a_file.size > int(settings.MAX_A_FILE_SIZE):
        raise ValidationError(u"파일의 크기가 너무 큽니다. 최대 %s. 현재 %s."%(filesizeformat(int(settings.MAX_A_FILE_SIZE)), filesizeformat(a_file.size)))
      totalsize = totalsize + a_file.size
    if totalsize > int(settings.MAX_TOTAL_FILE_SIZE):
      raise ValidationError(u"모든 파일의 총합이 너무 큽니다. 최대 %s. 현재 %s."%(filesizeformat(int(settings.MAX_TOTAL_FILE_SIZE)), filesizeformat(totalsize)))
