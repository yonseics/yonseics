...
urlpatterns = patterns('',
  (r'^$', main_page),                  # 메인 페이지
  (r'^qSearch/(?P<boardname>\w+)/$', bulletin_ac),  # 자동완성 테스트
  (r'^uSearch$', user_ac),              # 자동완성 테스트
  (r'^deleteTag/(?P<tid>\d+)/$', delete_tag_page_ajax),  # 태그 삭제(Ajax)
  ...
  (r'^read/(?P<bid>\d+)/$', direct_read),        # 게시물 번호로 직접 읽기
  (r'^comment/(?P<bid>\d+)/ajax_append/$', comment_append_ajax),      # 댓글달기(Ajax)
  (r'^comment/(?P<cid>\d+)/ajax_delete/$', comment_delete_ajax),      # 댓글삭제(Ajax)
